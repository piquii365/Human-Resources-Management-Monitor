...
-- Authentication stored procedure (basic register)
DROP PROCEDURE IF EXISTS sp_register;
DELIMITER $$
CREATE PROCEDURE sp_register(
  IN p_name VARCHAR(255), IN p_email VARCHAR(255), IN p_uid VARCHAR(255), IN p_displayPicture VARCHAR(1024)
)
BEGIN
  -- This procedure inserts a minimal user record into employees table for now; adapt to users/auth table when available
  INSERT INTO employees(id, first_name, last_name, email, created_at)
  VALUES (UUID(), p_name, '', p_email, NOW());
  -- In a full auth system you'd insert into a users table and handle password hashes, roles, etc.
END$$
DELIMITER ;
...
-- ----------------------------
-- Additional stored procedures for API usage
-- ----------------------------
...
DELIMITER $$
CREATE PROCEDURE sp_get_department_by_id(IN p_id CHAR(36))
BEGIN
...
CREATE PROCEDURE sp_create_department(
  IN p_id CHAR(36), IN p_name VARCHAR(255), IN p_code VARCHAR(64), IN p_description TEXT, IN p_head_employee_id CHAR(36)
)
BEGIN
  INSERT INTO departments(id, name, code, description, head_employee_id)
  VALUES (COALESCE(p_id, UUID()), p_name, p_code, p_description, p_head_employee_id);
END$$
DELIMITER ;
...
BEGIN
  UPDATE departments SET name = p_name, code = p_code, description = p_description, head_employee_id = p_head_employee_id, created_at = created_at
  WHERE id = p_id;
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE sp_delete_department(IN p_id CHAR(36))
BEGIN
...
DELIMITER $$
CREATE PROCEDURE sp_get_employee_by_id(IN p_id CHAR(36))
BEGIN
  SELECT * FROM employees WHERE id = p_id LIMIT 1;
END$$
DELIMITER ;
...
CREATE PROCEDURE sp_update_employee(
  IN p_id CHAR(36), IN p_user_id CHAR(36), IN p_employee_number VARCHAR(64), IN p_first_name VARCHAR(128), IN p_last_name VARCHAR(128), IN p_email VARCHAR(255), IN p_phone VARCHAR(50), IN p_department_id CHAR(36), IN p_position VARCHAR(255), IN p_hire_date DATE, IN p_employment_status ENUM('active','inactive','on_leave'), IN p_salary DECIMAL(12,2), IN p_photo_url VARCHAR(1024)
)
BEGIN
  UPDATE employees SET user_id = p_user_id, employee_number = p_employee_number, first_name = p_first_name, last_name = p_last_name, email = p_email, phone = p_phone, department_id = p_department_id, position = p_position, hire_date = p_hire_date, employment_status = p_employment_status, salary = p_salary, photo_url = p_photo_url, updated_at = NOW() WHERE id = p_id;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_delete_employee;
DELIMITER $$
CREATE PROCEDURE sp_delete_employee(IN p_id CHAR(36))
BEGIN
...
-- Recruitment CRUD
DROP PROCEDURE IF EXISTS sp_get_recruitments;
DELIMITER $$
CREATE PROCEDURE sp_get_recruitments()
BEGIN
  SELECT * FROM recruitment ORDER BY posting_date DESC;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_get_recruitment_by_id;
DELIMITER $$
CREATE PROCEDURE sp_get_recruitment_by_id(IN p_id CHAR(36))
BEGIN
  SELECT * FROM recruitment WHERE id = p_id LIMIT 1;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_create_recruitment;
DELIMITER $$
CREATE PROCEDURE sp_create_recruitment(
  IN p_id CHAR(36), IN p_job_title VARCHAR(255), IN p_department_id CHAR(36), IN p_description TEXT, IN p_requirements TEXT, IN p_position_type ENUM('full_time','part_time','contract'), IN p_salary_range VARCHAR(128), IN p_posting_date DATE, IN p_closing_date DATE, IN p_status ENUM('open','closed','filled'), IN p_vacancies INT, IN p_created_by CHAR(36)
)
BEGIN
  INSERT INTO recruitment(id, job_title, department_id, description, requirements, position_type, salary_range, posting_date, closing_date, status, vacancies, created_by)
  VALUES (COALESCE(p_id, UUID()), p_job_title, p_department_id, p_description, p_requirements, p_position_type, p_salary_range, p_posting_date, p_closing_date, p_status, p_vacancies, p_created_by);
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_update_recruitment;
DELIMITER $$
CREATE PROCEDURE sp_update_recruitment(
  IN p_id CHAR(36), IN p_job_title VARCHAR(255), IN p_department_id CHAR(36), IN p_description TEXT, IN p_requirements TEXT, IN p_position_type ENUM('full_time','part_time','contract'), IN p_salary_range VARCHAR(128), IN p_posting_date DATE, IN p_closing_date DATE, IN p_status ENUM('open','closed','filled'), IN p_vacancies INT
)
BEGIN
  UPDATE recruitment SET job_title = p_job_title, department_id = p_department_id, description = p_description, requirements = p_requirements, position_type = p_position_type, salary_range = p_salary_range, posting_date = p_posting_date, closing_date = p_closing_date, status = p_status, vacancies = p_vacancies, updated_at = NOW() WHERE id = p_id;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_delete_recruitment;
DELIMITER $$
CREATE PROCEDURE sp_delete_recruitment(IN p_id CHAR(36))
BEGIN
  DELETE FROM recruitment WHERE id = p_id;
END$$
DELIMITER ;

-- Applications list/get/update
DROP PROCEDURE IF EXISTS sp_get_applications_by_recruitment;
DELIMITER $$
CREATE PROCEDURE sp_get_applications_by_recruitment(IN p_recruitment_id CHAR(36))
BEGIN
  SELECT * FROM applications WHERE recruitment_id = p_recruitment_id ORDER BY application_date DESC;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_get_application_by_id;
DELIMITER $$
CREATE PROCEDURE sp_get_application_by_id(IN p_id CHAR(36))
BEGIN
  SELECT * FROM applications WHERE id = p_id LIMIT 1;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_update_application_status;
DELIMITER $$
CREATE PROCEDURE sp_update_application_status(IN p_id CHAR(36), IN p_status ENUM('pending','reviewed','shortlisted','rejected','hired'))
BEGIN
  UPDATE applications SET status = p_status, updated_at = NOW() WHERE id = p_id;
END$$
DELIMITER ;

-- Training programs and enrollments
DROP PROCEDURE IF EXISTS sp_get_training_programs;
DELIMITER $$
CREATE PROCEDURE sp_get_training_programs()
BEGIN
  SELECT * FROM training_programs ORDER BY start_date DESC;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_get_training_by_id;
DELIMITER $$
CREATE PROCEDURE sp_get_training_by_id(IN p_id CHAR(36))
BEGIN
  SELECT * FROM training_programs WHERE id = p_id LIMIT 1;
END$$
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_get_enrollments_by_program;
DELIMITER $$
CREATE PROCEDURE sp_get_enrollments_by_program(IN p_program_id CHAR(36))
BEGIN
  SELECT te.*, e.first_name, e.last_name FROM training_enrollments te JOIN employees e ON te.employee_id = e.id WHERE te.training_program_id = p_program_id ORDER BY te.enrollment_date DESC;
END$$
DELIMITER ;

-- Calendar events: combine application interviews and training start/end dates
DROP PROCEDURE IF EXISTS sp_get_calendar_events;
DELIMITER $$
CREATE PROCEDURE sp_get_calendar_events(IN p_from DATE, IN p_to DATE)
BEGIN
  -- Interview events
  SELECT id AS event_id, recruitment_id AS related_id, applicant_name AS title, interview_date AS event_date, 'interview' AS event_type
  FROM applications
  WHERE interview_date IS NOT NULL AND DATE(interview_date) BETWEEN p_from AND p_to
  UNION ALL
  -- Training start events
  SELECT id AS event_id, id AS related_id, title AS title, start_date AS event_date, 'training_start' AS event_type
  FROM training_programs
  WHERE start_date BETWEEN p_from AND p_to
  UNION ALL
  -- Training end events
  SELECT id AS event_id, id AS related_id, title AS title, end_date AS event_date, 'training_end' AS event_type
  FROM training_programs
